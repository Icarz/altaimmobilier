...
import { listings, getListingById, CATEGORY_LABELS } from '../../data/mockData'
...
export default function EditListing() {
  ...
  const original = getListingById(id)

  if (!original) return <Navigate to="/admin/listings" replace />

  const [form,    setForm]    = useState({ ...original })
  const [photos,  setPhotos]  = useState(
    original.photos.map(p => ({ ...p, isNew: false, preview: p.url }))
  )
  ...
  function removePhoto(photoId) {
    ...
    setPhotos(p => {
      ...
      const hadCover = p.find(ph => ph.id === photoId)?.is_cover
      if (hadCover && next.length > 0) next[0] = { ...next[0], is_cover: true }
  ...
  // Drag-to-reorder (simple swap on drag-end)
  ...
  function onDrop(targetId) {
    ...
    setPhotos(p => {
      const from = p.findIndex(ph => ph.id === dragging)
      const to   = p.findIndex(ph => ph.id === targetId)
      const next = [...p]
  ...
  function handleSubmit(e) {
    ...
    // In production: PUT /api/admin/listings/:id
    ...
    if (idx !== -1) {
      listings[idx] = {
        ...
        photos:       photos.map((ph, i) => ({
          id: ph.id, url: ph.url, is_cover: ph.is_cover, display_order: i,
        })),
      }
    }
    setSuccess(true)
  ...
  return (
    <AdminLayout title={`Modifier — ${original.title}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-clay-100 p-6 space-y-5">
              ...
              <Field label="Description" required error={errors.description}>
                ...
                <p className="font-sans text-xs text-clay-300 text-right mt-1">
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                {photos.map(ph => (
                  <div
                    ...
                    className={`relative group rounded-lg overflow-hidden aspect-square bg-clay-100 cursor-grab active:cursor-grabbing
                      ${dragging === ph.id ? 'ring-2 ring-terra opacity-60' : ''}`}
                  >
                ...
                {photos.length < 15 && (
                  <label className="aspect-square rounded-lg border-2 border-dashed border-clay-200
                                    ...
                                    hover:border-terra transition-colors">
            ...
            <div className="flex flex-col gap-3">
              ...
              <Link to="/admin/listings"
                className="w-full py-2.5 text-clay-400 font-sans text-sm text-center
                           hover:text-clay-600 transition-colors">
                ← Retour aux annonces
              </Link>
            </div>
          </div>
